 ...
 * net.jcards.eval.HandWithSharedIterator, however, it is much faster in
 * performing those same functions therefore it should be used whenever
 * possible.
 */
...
public class HandIterator implements Iterator<Long> {
	...
	//private static final Logger _log = Logger.getLogger(HandIterator.class);
	...
	public HandIterator(int aNumberOfCards) {
		...
		if(aNumberOfCards < 0 || aNumberOfCards > 7) {
			throw new ArgumentOutOfRangeException(
					"this iterator only supports 0-7 cards");
		}
		...
		if(_log.isTraceEnabled()) {
			_log.trace("binomial: " + Hand.NumberOfCards + " choose " + _numberOfCards);
		}
		*/
	}
	...
	/**
	 * Evaluates all possible combinations of 7 cards from a typical card deck
	 * (usually 52 cards).
	 *
